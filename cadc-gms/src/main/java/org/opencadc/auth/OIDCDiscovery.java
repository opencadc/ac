...
/**
 * Cache object that manages the cache file for the OIDC Discovery (/.well-known) document.
 */
public class OIDCDiscovery {
    ...
    public CachingFile getCachingFile() throws MalformedURLException {
        return new CachingFile(getCachedFile(), new URL(issuer.toURL().toExternalForm() + OIDCDiscovery.WELL_KNOWN_ENDPOINT));
    }
    ...
    private Path getBaseCacheDirectory() {
        ...
        if (userName == null) {
            ...
            baseCacheDir = Paths.get(tmpDir, userName, OIDCDiscovery.CACHE_DIRECTORY_NAME);
        }
