...
public class GroupManagementIntTest {
    ...
    private void getGroups(final OutputStream outputStream, final String contentType, final String[] groupURIs,
                           final int[] gids) throws Throwable {
        final InputStreamWrapper inputStreamWrapper = inputStream -> outputStream.write(inputStream.readAllBytes());
        final StringBuilder urlBuilder = new StringBuilder(groupMapperURL.toString());

        if (groupURIs.length > 0 || gids.length > 0) {
            urlBuilder.append("?");
        }

        if (groupURIs.length > 0) {
            urlBuilder.append("group=");
            urlBuilder.append(String.join("&group=", groupURIs));
        }
